...
trait SwingApi {
  ...
  implicit class TextFieldOps(field: TextField) {
    ...
    /** Returns a stream of text field values entered in the given text field.
      ...
      */
    def textValues: Observable[String] = Observable{ob=> {
      field.subscribe { case ValueChanged(x)=> ob.onNext(x.text) }
    }
    }
  ...
  implicit class ButtonOps(button: Button) {
     ...
     */
    def clicks: Observable[Button] = Observable{ob=> {
      button.subscribe { case ButtonClicked(x)=> ob.onNext(x) }
    }
    }
  }
