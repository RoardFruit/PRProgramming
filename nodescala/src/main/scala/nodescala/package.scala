...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = fs match{
      case Nil=>Future(Nil)
      case head::tail=>head.flatMap(x=>all(tail).map(y=>x::y))
    }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
     ...
     */
    def now: T = try {Await.result(f,0 nanos)} catch { case _:TimeoutException=>throw new (NoSuchElementException)}
     ...
     */
    def continueWith[S](cont: Future[T] => S): Future[S] = f.map(x=>cont(f))
     ...
     *  The resulting future contains a value returned by `cont`.
     */
    def continue[S](cont: Try[T] => S): Future[S] = f.map(x=>cont(Try(x)))
